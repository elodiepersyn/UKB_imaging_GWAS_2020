...
ymax<-max(-log10(data$V16)+1.5)
topsnp<-data[which(data$SNP==SNP.name),4]
topp<-data[which(data$SNP==SNP.name),16]
...
plot(frame=F,data_nor2$V3,-log10(data_nor2$V16),bg=ifelse(data_nor2$R2>0.8,cols[1],ifelse(data_nor2$R2>0.6,cols[2],ifelse(data_nor2$R2>0.4,cols[3],ifelse(data_nor2$R2>0.2,cols[4],"grey")))),pch=21, col=ifelse(data_nor2$R2>0.2,"black","grey"),ylab="",xlab="",ylim=c(0,ymax),xlim=c(BP_min,BP_max),xaxt='n',yaxt='n')
par(new=T)
plot(frame=F,data_r2$V3,-log10(data_r2$V16),
